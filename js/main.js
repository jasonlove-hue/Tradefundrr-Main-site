...
(function () {
  ...
  // Compensate the fixed nav for the vertical scrollbar width so it frames the
  // page content box: logo on the content's left edge, Get Funded on the right.
  // (CSS calc(100vw - 100%) can't see the scrollbar for a fixed element.)
  const setNavGutter = () => {
    const sbw = window.innerWidth - document.documentElement.clientWidth;
    document.documentElement.style.setProperty('--sbw', (sbw > 0 ? sbw : 0) + 'px');
  };
  setNavGutter();
  window.addEventListener('resize', setNavGutter, { passive: true });
  window.addEventListener('load', setNavGutter);
